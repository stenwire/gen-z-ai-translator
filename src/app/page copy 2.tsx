      ...
      <div className="bg-white/30 backdrop-blur-lg p-8 rounded-2xl shadow-2xl max-w-6xl w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {state.translations?.map((translation, index) => {
            const parts = translation.split(' Gen Z: ');
            let original = '';
            let genz = translation;
            if (parts.length === 2) {
              original = parts[0].startsWith('Original: ') ? parts[0].substring('Original: '.length) : parts[0];
              genz = parts[1];
            }
            ...
            return (
              ...
              >
                ...
                <hr></hr>
                <br></br>
                <h3 className="text-sm font-semibold mb-1">Gen Z Translation</h3>
                <p className="text-sm flex-grow">{genz}</p>
                <div className="absolute right-2 top-2 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => copyToClipboard(genz)}
